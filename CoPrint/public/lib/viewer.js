...
setBullseyePosition = function(x, y, z) {
    ...
    function init3D() {
            ...
            if (webgl) {
                printLog('<h5><i class="fa fa-search fa-fw" aria-hidden="true"></i>WebGL Support found!</h5><b>success:</b><br> App will work optimally on this device!<hr><p>', successcolor);
                renderer = new THREE.WebGLRenderer({
                    // autoClearColor: true,
                    antialias: false,
                    alpha: true
        ...
        $(window).on('resize', function() {
            ...
            camera.updateProjectionMatrix();
            controls.reset();
            setTimeout(function(){   resetView(); }, 100);
        ...
        function resetView() {
          if (scene.children.length > 1) {
              ...
              for (i = 1; i < scene.children.length; i++) {
                var object = scene.children[i].clone();
